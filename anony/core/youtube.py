...
# ✅ Changed: use V2 client from fallenapi.py (module-level client)
...
class YouTube:
    ...
    def get_cookies(self):
        # ✅ small safety: do not crash if folder doesn't exist
        if not self.checked:
            try:
    ...
    async def download(self, video_id: str, video: bool = False) -> Optional[str]:
        ...
        # ✅ ONLY changed API path to match fallenapi.py (module-level client)
        if config.API_URL and config.API_KEY:
            try:
                file_path = await v2_client.download(url, isVideo=video, title=video_id)
                if file_path:
                    return file_path
            except Exception as e:
                logger.warning(f"[V2] API download failed, using yt-dlp fallback: {e}")

        # ✅ yt-dlp fallback (kept same as your file)
